...
# Cabecera UDP de video enviada por web_server_rover.cpp:
# struct packed UdpVideoPacketHeader {
#   uint16_t magic;        // 0x5256 = 'RV'
#   uint16_t frameId;
#   uint16_t packetIndex;
#   uint16_t packetCount;
#   uint16_t payloadSize;
# }
# En ESP32 estos uint16_t salen en little-endian.
UDP_MAGIC = 0x5256
UDP_HEADER_FORMAT_LE = "<HHHHH"
UDP_HEADER_FORMAT_BE = "!HHHHH"  # compatibilidad con visores/firmwares antiguos
UDP_HEADER_SIZE = struct.calcsize(UDP_HEADER_FORMAT_LE)
    ...
    def udp_stream_worker(self):
        ...
        try:
            ...
            while self.running and self.using_udp_transport():
                ...
                try:
                    # El transmisor ESP32 escribe la struct binaria tal cual,
                    # por lo que llega en little-endian. Dejamos un fallback
                    # big-endian para no romper firmwares antiguos.
                    magic, frame_id, packet_index, packet_count, payload_size = struct.unpack(
                        UDP_HEADER_FORMAT_LE,
                        packet[:UDP_HEADER_SIZE]
                    )

                    if magic != UDP_MAGIC:
                        magic, frame_id, packet_index, packet_count, payload_size = struct.unpack(
                            UDP_HEADER_FORMAT_BE,
                            packet[:UDP_HEADER_SIZE]
                        )
                except struct.error:
                    continue
